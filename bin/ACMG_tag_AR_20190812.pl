...
$file2="SNV_allsites_ACMG_AR.tsv";
open FILE1,"<$file1" or die "Can't open $file1 :$!\n";
open FILE2,">$file2" or die "Can't open $file2 :$!\n";
print FILE2 "chrom\tpos\tref\talt\tgnomAD_AF_SAS_WES\tgnomAD_AF_SAS_WES\tgnomAD_grpmax_WES\tgnomAD_grpmax_WGS\tgnomAD_AF_EAS_WES\tgnomAD_AF_ASJ_WES\tgnomAD_AF_MID_WES\tgnomAD_AF_FIN_WES\tgnomAD_AF_NFE_WES\tgnomAD_AF_AFR_WES\tDBNSFP_INTERPRO_DOMAIN\tVEP_MaxEntScan_diff\tVEP_CONSEQUENCE\tVEP_DOMAINS\ttranscript\tGNOMAD_AF_AMI_WES\tVEP_HGVSC\tVEP_HGVSP\tVEP_PICK\tVEP_GENE\tVEP_SYMBOL\tCLINVAR_CLNSIG\tDVD_PATHOGENICITY\tDBNSFP_REVEL_SCORE\trmsk_repName\tPP5\tPM2\tBA1\tBS1\tPM4\treplace1\treplace2\tBP3\tPP7\tBA2\n";
my ($pp5,$pm2_2,$pm2_3,$ba1,$bs1_1,$bs1_3,$pm4,$pp3,$bp4,$pp7,$bp3)=();
while(my $line=<FILE1>){
	$line=~s/[\r\n]//g;
	next if($line=~/CHROM/);
	my @line_split=split /\t/,$line;
...
#		}
	...
	elsif($maxaf >= 0.003 && $maxaf < 0.005){
		$line_split[31]=1;
		$bs1_1++;
	}
	elsif($maxaf >= 0.0007 && $maxaf < 0.003){
		$line_split[31]=3;
		$bs1_3++;
	}
	elsif($maxaf > 0.00007 && $maxaf < 0.0007){
		$line_split[29]=3;
		$pm2_3++;
	}
	elsif($maxaf <=0.00007){
