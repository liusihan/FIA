...
cat ACMG_classfy_results_filtered.1.tsv | awk 'BEGIN{FS="\t";OFS="\t"}NR==FNR&&FNR==1{for(i=6;i<=NF;i++){ix[i]=$i}}NR==FNR&&NR>=2{for(i=6;i<=NF;i++){if($i!="NA"){a[$1$2$3$4$5]=a[$1$2$3$4$5]""";"""ix[i]"="$i}}}NR>FNR{gsub(/PP5=[0-9]+;?/, "", $29);gsub(/PP5=[0-9]+;?/, "", $25);if($(NF-1)!~/missense/&&$25~/PS1/&&$(NF-1)!~/splice/){gsub(/PS1=[0-9]+;?/, "", $25);gsub(/PS1=[0-9]+;?/, "", $29)};if($(NF-1)~/synonymous/&&$25!~/PP3/){$25=$25";BP7=3";$29=$29";BP7=3"};if(substr($25,length($25))==";"){$25=substr($25,1,length($25)-1)};if(substr($29,length($29))==";"){$29=substr($29,1,length($29)-1)};if(substr($25,1,1)==";"){$25=substr($25,2)};if(substr($29,1,1)==";"){$29=substr($29,2)};gsub(/;;/, ";", $25);gsub(/;;/, ";", $29);if($1$2$3$4$5 in a){$29=$29""a[$1$2$3$4$5];$25=$25""a[$1$2$3$4$5];print $0}else{print $0}}' $Manuel_evidence - | awk 'BEGIN{FS="\t";OFS="\t"}NR==FNR{a[$1]=0}NR>FNR{if($30 in a){gsub(/BA1=[0-9]+;?/, "", $25);gsub(/BA1=[0-9]+;?/, "", $29);if(substr($25,length($25))==";"){$25=substr($25,1,length($25)-1)};if(substr($29,length($29))==";"){$29=substr($29,1,length($29)-1)};if(substr($25,1,1)==";"){$25=substr($25,2)};if(substr($29,1,1)==";"){$29=substr($29,2)};gsub(/;;/, ";", $25);gsub(/;;/, ";", $29);print $0}else{print $0}}' $input_dir/BA1_exclusion_list.txt - >ACMG_classfy_results_filtered.final.tsv
